...
private:
    ...
    char d_letter;
public:
Angle(int d, float min, char dl): degree(d), minute(min),d_letter(dl){}
...
void display()
{
 cout <<degree<<"\xF8"<<fixed<< setprecision(1) <<minute<<"'"<<d_letter<<endl;}
};

int main ()
{
Angle A1(158,60.2,'N');
cout <<"initialize angle: ";
A1.display();
Angle user(0,0.0,'N');//default 
while(true){
    user.obtainAngle();
